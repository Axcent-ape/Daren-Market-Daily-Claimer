        ...
        if await daren.login():
            ...
            if not await daren.check_task(task_id="DAILY_CHECK_IN", claimed=True) and await daren.check_task(task_id="DAILY_CHECK_IN", claimed=False):
                if await daren.daily_check_in():
                    logger.success(f"Поток {thread} | Выполнил check-in {private_key}")
                else:
                    logger.error(f"Поток {thread} | Не выполнил check-in {private_key}")
            else:
                logger.warning(f"Поток {thread} | Уже выполнил check-in {private_key}")
            ...
            if not await daren.check_task(task_id="OP_BNB_CHECK_IN", claimed=True) and not await daren.check_task(task_id="OP_BNB_CHECK_IN", claimed=False):
                ...
                if status:
                    ...
                    logger.success(f"Поток {thread} | Не выполнил opBNB check-in {daren.web3_utils.acct.address}:{tx_hash}")
            else:
                logger.warning(f"Поток {thread} | Уже выполнил opBNB check-in {private_key}")
